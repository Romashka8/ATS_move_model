...
if __name__ == "__main__":
    ...
    if mode == "stab":
        ...
        for i in range(start, finish + 1):
            print(i)
            h_arr, r_arr = mb.generate_arrays(i, 0.7, 3)
            mb.rec_stab(4000, h_arr, r_arr)
            RobotATS.delayed = []
            mb.save_conf(f"{i}", f"res/{cluster}/07/{start}_{finish}.bin", roads[3], h_arr, r_arr)
            roads[3].clear_road()
    if mode == "stat":
        input()
        start_time = time.time()
        for i in range(start, finish + 1):
            print(i)
            v.reset_v()
            rm, h_arr, r_arr = mb.load_conf(f"{i}", f"res/{cluster}/07/{start}_{finish}.bin")
            roads[3] = rm
            RobotATS.delayed = []
            mb.rec_stat(5000, h_arr, r_arr)
            v.v = v.v / (5000 * i)
            mb.save_stat(i / 1000, v.v, f"res/{cluster}/07/{start}_{finish}.txt")
